...
function renderInactive() {
    ...
    el.className = 'center-box';
    el.innerHTML = `
        <div class="error-404" style="font-size:3rem;">
            ⌛︎
            <span class="frown" style="font-size:1.5rem;">link expired or not yet active</span>
        </div>
    `;
    return el;
}
        ...
        for (const k of filteredKeys) {
            ...
            if (rawVal) {
                ...
                if (typeof destRaw === 'string') {
                    ...
                    // keep 'simple' for direct URLs and k: references
                }
            }
...
function renderQRCommand(key) {
    ...
    // ── Build the QR data: use the query parameter version ──
...
function init() {
    ...
    if (keyExists(key)) {
        ...
        if (!isLinkActive(rawVal)) {
            app.appendChild(renderInactive());
            return;
        }
